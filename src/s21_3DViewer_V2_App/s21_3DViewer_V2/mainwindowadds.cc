...
void MainWindow::on_MainWindow_iconSizeChanged(const QSize &iconSize) {
  if (iconSize.width() > 100)
    emit send_main_window_size_changed(iconSize);
}
...
void MainWindow::read_start_pos() {
  ...
  float RedCL = 0.8, GreenCL = 0.5, BlueCL = 0.2, RedCV = 0.5, GreenCV = 0.8,
        BlueCV = 0.2;
  ...
  if ((start_pos = fopen("start_pos-v2.pos", "r")) == NULL) {
    ...
    fprintf(start_pos, "%.2lf %.2lf %.2lf %.2lf %.2lf %.2lf", 0.8, 0.5, 0.2,
            0.5, 0.8, 0.2);
    fprintf(start_pos, "%d %d %d %d %d %d ", 0, 0, 0, 0, 0, 0);
    fprintf(start_pos, "%d %d %d %d %d ", 1, 1, 0, 0, 0);
  } else {
    char Multip_str[20] = {0}, RedC_str[20] = {0}, GreenC_str[20] = {0},
         BlueC_str[20] = {0};
    char RedCL_str[20] = {0}, GreenCL_str[20] = {0}, BlueCL_str[20] = {0},
         RedCV_str[20] = {0}, GreenCV_str[20] = {0}, BlueCV_str[20] = {0};
    fscanf(start_pos, "%s%s%s%s", Multip_str, RedC_str, GreenC_str,
                    BlueC_str);
    fscanf(start_pos, "%s%s%s%s%s%s", RedCL_str, GreenCL_str,
                    BlueCL_str, RedCV_str, GreenCV_str, BlueCV_str);
    fscanf(start_pos, "%d%d%d%d%d%d", &xRot, &yRot, &zRot, &xMove,
                    &yMove, &zMove);
    fscanf(start_pos, "%d%d%d%d%d", &LineWidth, &VertexSize,
                    &VertexType, &perspect, &lineType);
  ...
  fclose(start_pos);
  ui->MultipSlider->setValue((int)(Multip * 100));
  ui->zMoveSlider->setSliderPosition((int)Multip);
  ui->MultipSlider->repaint();
  ui->SBRedColor->setValue((int)(RedC * 255));
  ui->SBRedColor->repaint();
  ui->SBGreenColor->setValue((int)(GreenC * 255));
  ui->SBGreenColor->repaint();
  ui->SBBlueColor->setValue((int)(BlueC * 255));
  ui->SBBlueColor->repaint();
  ui->SBRedColorLine->setValue((int)(RedCL * 255));
  ui->SBRedColorLine->repaint();
  ui->SBGreenColorLine->setValue((int)(GreenCL * 255));
  ui->SBGreenColorLine->repaint();
  ui->SBBlueColorLine->setValue((int)(BlueCL * 255));
  ui->SBBlueColorLine->repaint();
  ui->SBRedColorVertex->setValue((int)(RedCV * 255));
  ui->SBRedColorVertex->repaint();
  ui->SBGreenColorVertex->setValue((int)(GreenCV * 255));
  ui->SBGreenColorVertex->repaint();
  ui->SBBlueColorVertex->setValue((int)(BlueCV * 255));
  ui->SBBlueColorVertex->repaint();
  ui->xSlider->setValue(xRot);
  ui->xSlider->setSliderPosition(xRot);
  ui->xSlider->repaint();
  ui->ySlider->setValue(yRot);
  ui->ySlider->setSliderPosition(yRot);
  ui->ySlider->repaint();
  ui->zSlider->setValue(zRot);
  ui->zSlider->setSliderPosition(zRot);
  ui->zSlider->repaint();
  ui->xMoveSlider->setValue(xMove);
  ui->xMoveSlider->setSliderPosition(xMove);
  ui->xMoveSlider->repaint();
  ui->yMoveSlider->setValue(yMove);
  ui->yMoveSlider->setSliderPosition(yMove);
  ui->yMoveSlider->repaint();
  ui->zMoveSlider->setValue(zMove);
  ui->zMoveSlider->setSliderPosition(zMove);
  ui->zMoveSlider->repaint();
  ui->SBLineWidth->setValue(LineWidth);
  ui->SBLineWidth->repaint();
  ui->SBVertexSize->setValue(VertexSize);
  ui->SBVertexSize->repaint();
  ui->CBVertexType->setCurrentIndex(VertexType);
  ui->CBVertexType->repaint();
  if (perspect == 1) {
...
void MainWindow::on_PBToZero_clicked() {
  ...
  ui->SBVertexSize->repaint();
  ui->CBVertexType->setCurrentIndex(VertexType);
  std::cout << "VertexType on load = " << VertexType << std::endl;
  ui->CBVertexType->repaint();
  if (perspect == 1) {
    ui->CBPerspective->setChecked(true);
  } else {
