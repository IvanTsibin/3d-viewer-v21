...
Cube::Cube()
    : m_count(0)
{
//    m_data.resize(10000 * 6);
//    m_dot_data.resize(20000 * 6);
    figure_t figure;
    creat_cube(&figure);
...
void Cube::reLoadData(const char file_name[]) {
...
//    std::cout << "ReLoad Step - 3" << std::endl;
    ...
    for (int i = 0; i < temp_figure.lines_number; i++) {
        for (int j = 0; j < 2; j++) {
            ...
            *p++ = temp_figure.dots[temp_figure.lines[i][j]][1];
...
void Cube::init_figure(figure_t *figure) {
    ...
    if (figure->triangles_number > 6000000) figure->triangles_number = 6000000;
    ...
    if (figure->lines_number > 18000000) figure->lines_number = 18000000;
...
void Cube::remove_figure(figure_t *figure) {
...
void Cube::print_figure(figure_t *figure) {
//    for (int i = 0; i < figure->dots_number; i++) {
...
void Cube::copy_figure(figure_t *dist_figure, figure_t *src_figure) {
...
void Cube::resize_and_move_figure(figure_t *temp_figure) {
...
void Cube::move_figure(figure_t *figure, double x, double y, double z) {
    for (int i = 0; i < figure->dots_number; i++) {
...
void Cube::resize_figure(figure_t *figure, double x, double y, double z) {
    for (int i = 0; i < figure->dots_number; i++) {
        figure->dots[i][0] = figure->dots[i][0] * x;
        figure->dots[i][1] = figure->dots[i][1] * y;
        figure->dots[i][2] = figure->dots[i][2] * z;
...
int Cube::read_data_from_file(const char file_name[], figure_t *figure) {
...
void Cube::read_number_of_dots_and_facets_from_str(char **strs_from_file, int str_number, figure_t *figure) {
    figure->dots_number = 0;
    figure->triangles_number = 0;
...
int Cube::number_of_lines_from_file(FILE *obj_file) {
     char letter;
     int str_counter = 0;
     while (!feof(obj_file)) {
...
void Cube::find_lines_number(figure_t *figure) {
...
void Cube::creat_hash_for_lines_number(int **temp_lines, int counter, int **hash_arr, int hash_amount) {
    int counter_3 = 0;
...
void Cube::print_str_from_file(char **strs_from_file, int str_number) {
...
void Cube::read_dots_from_str(char **strs_from_file, int str_number, figure_t *figure) {
    int counter = 0;
        for (int i = 0; i < str_number; i++) {
...
double Cube::convert_str_to_double(char number[], int counter) {
...
void Cube::read_polygons_from_str(char **strs_from_file, int str_number, figure_t *figure, int *lines_lenths) {
...
void Cube::read_str_from_file(FILE *obj_file, char **strs_from_file, int *str_number) {
     char letter;
     int counter = 0;
     while (!feof(obj_file)) {
         letter = fgetc(obj_file);
...
void Cube::read_line_lenth(FILE *obj_file, int *line_lenths) {
